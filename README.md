# Tracking Covid-19 Cases over time in Erie County, NY by ZIP Code
This project started as I found the Covid-19 dashboard run by Erie County hard to read and not conveying the information I found most important (where are cases trending upwards? How do the current new cases compare to other ZIP codes?). I built a data scraper in Python that collected data from the Erie County website, analyzed growth factors, and restructured these variables with SQL to be read by a Tableau dashboard. This allowed users to select individual ZIP codes and compare case growth over time, as well as compare it with census data. This revealed several trends, mainly: cases were higher initially in ZIP codes where nursing homes existed/the average age of the population was higher. It also revealed that ZIP codes with predominantly people of color were disproportionately affected. To investigate these trends further, a Freedom of Information Act request was submitted to the Erie County Department of Health, which was repeatedly denied. Contains the only source for ZIP code-level Covid-19 data for Erie county collected over the period of two years. Featured on, and used by The Buffalo News. 
